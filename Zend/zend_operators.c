...
static int compare_long_to_string(zend_long lval, zend_string *str, zend_bool equality_only, zval *orig_op1, zval *orig_op2) /* {{{ */
{
	zend_long str_lval;
	double str_dval;
	int num_cmp_result;
	...
	if (type == IS_LONG) {
		...
		compare_function_orig(num_cmp_result, orig_op1, orig_op2);
	}
...
static int compare_double_to_string(double dval, zend_string *str, zend_bool equality_only, zval *orig_op1, zval *orig_op2) /* {{{ */
{
	zend_long str_lval;
	double str_dval;
	int num_cmp_result;
	...
	while (1) {
		switch (TYPE_PAIR(Z_TYPE_P(op1), Z_TYPE_P(op2))) {
			...
			case TYPE_PAIR(IS_LONG, IS_STRING):
				ZVAL_LONG(result, compare_long_to_string(Z_LVAL_P(op1), Z_STR_P(op2), equality_only, op1, op2));
				return SUCCESS;

			case TYPE_PAIR(IS_STRING, IS_LONG):
				ZVAL_LONG(result, -compare_long_to_string(Z_LVAL_P(op2), Z_STR_P(op1), equality_only, op1, op2));
				return SUCCESS;
			...
			case TYPE_PAIR(IS_DOUBLE, IS_STRING):
				...
				ZVAL_LONG(result, compare_double_to_string(Z_DVAL_P(op1), Z_STR_P(op2), equality_only, op1, op2));
				return SUCCESS;
			...
			case TYPE_PAIR(IS_STRING, IS_DOUBLE):
				...
				ZVAL_LONG(result, -compare_double_to_string(Z_DVAL_P(op2), Z_STR_P(op1), equality_only, op1, op2));
				return SUCCESS;
...
ZEND_API int ZEND_FASTCALL compare_function_orig(zval *result, zval *op1, zval *op2) /* {{{ */
{
	int ret;
	int converted = 0;
	zval op1_copy, op2_copy;
